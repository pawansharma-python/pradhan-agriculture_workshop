...
const defaultProducts = [
    { id: 1, name: "Premium Red Seed Drill (11 Tyner)", category: "Seed Drill", badge: "Best Seller", desc: "Heavy-duty Seed Drill for precise sowing and fertilizing. Designed for maximum yield.", image: "https://images.unsplash.com/photo-1628543105335-512ce24e8371?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80", price: 65000, discount: 10 },
    { id: 2, name: "Advanced Paddy Cleaner", category: "Paddy Cleaner", badge: "Top Rated", desc: "Baba Ahejati Pradhan Paddy Cleaner. Heavy motor to separate dust and stones efficiently.", image: "https://images.unsplash.com/photo-1589939705384-5185137a7f0f?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80", price: 45000, discount: 5 },
    { id: 3, name: "Heavy Tractor Front Loader", category: "Tractor Attachment", badge: "Heavy Duty", desc: "Solid built yellow loader suitable for all major tractor brands. Moving heavy soil and sand.", image: "https://images.unsplash.com/photo-1590496794008-383c8070b257?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80", price: 125000, discount: 15 },
    { id: 4, name: "Compact Seed Drill (9 Tyner)", category: "Seed Drill", badge: "Economy", desc: "Perfect for small to medium fields. High precision planting technology.", image: "https://images.unsplash.com/photo-1586771107445-d3ca888129ff?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80", price: 55000, discount: 0 },
    { id: 5, name: "Multi-Crop Thresher", category: "Thresher", badge: "New Arrival", desc: "Separate grains seamlessly. High capacity thresher for wheat, maize, and paddy.", image: "https://images.unsplash.com/photo-1530983821634-60c793ba67df?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80", price: 180000, discount: 5 }
];

let products = JSON.parse(localStorage.getItem('paw_final_v4')) || defaultProducts;
let cart = JSON.parse(localStorage.getItem('paw_cart_v4')) || [];

let currentFilteredList = [];
let displayedCount = 0;
const ITEMS_PER_PAGE = 6;

// --- URL HASH ROUTING (SECRET PORTAL) ---
...
function loadMoreProducts() {
    ...
    loadBtn.style.display = displayedCount >= currentFilteredList.length ? 'none' : 'block';
...
function addToCart(id) {
    ...
    localStorage.setItem('paw_cart_v4', JSON.stringify(cart));
    updateCartUI();
...
function renderAdminTable() {
    ...
    localStorage.setItem('paw_final_v4', JSON.stringify(products));
}
...
document.getElementById('productForm').addEventListener('submit', function (e) {
    ...
    if (idInput) {
        ...
        products[index] = { id: parseInt(idInput), name, category, desc, image, price, discount, badge };
        document.getElementById('submit-btn').innerHTML = '<i class="fas fa-plus-circle"></i> Publish to Catalog';
    } else {
        const newId = products.length > 0 ? Math.max(...products.map(p => p.id)) + 1 : 1;
        products.push({ id: newId, name, category, desc, image, price, discount, badge });
    }
...
function deleteProduct(id) {
    Swal.fire({ title: 'Delete Machine?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#DD2C00', confirmButtonText: 'Delete' }).then((result) => {
        if (result.isConfirmed) {
            products = products.filter(p => p.id !== id); renderAdminTable();
